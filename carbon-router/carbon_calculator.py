...
with open(os.path.join(os.path.dirname(__file__), "servers.json"), "r", encoding="utf-8") as f:
...
def _get_watttime_token() -> str | None:
    ...
    print(f"WattTime login status: {rsp.status_code}, Response: {rsp.text}")
...
def extract_text_from_pdf(upload: Any) -> str:
...
def estimate_tokens_from_pdf(upload: Any) -> int:
...
async def analyze_pdf_carbon_impact(pdf_file: Any) -> Dict[str, Any]:
    tokens_needed = estimate_tokens_from_pdf(pdf_file)

    print(f"USE_WATTTIME: {USE_WATTTIME}, WATTTIME_USERNAME: {WATTTIME_USERNAME is not None}, WATTTIME_PASSWORD: {WATTTIME_PASSWORD is not None}")
    ...
    for server in SERVER_CONFIG["servers"]:
        if using_mock:
            ...
            try:
                # Force CAISO_NORTH region as per user request
                region = "CAISO_NORTH"
                carbon_intensity = _get_current_moer(token, region)
            except requests.HTTPError:
                # If intensity fails, skip server
                continue
